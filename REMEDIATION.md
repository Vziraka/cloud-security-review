...
| --- | --- | --- | --- |
| F1 | Immediately; Security Operations, account custodian, incident lead | Verify root activity through a trusted channel; preserve raw audit and identity records; secure credentials/recovery and contain unauthorized access under the incident plan. Escalate suspected compromise to leadership; involve privacy/legal if data exposure is suspected. | Incident lead records an evidence-supported disposition, containment/recovery where needed, and residual follow-up owners; current MFA/credential posture, sessions and persistence reviewed; root-use alert received and acknowledged. Escalation alone does not close the finding. Test alerts without an unnecessary real root login. |
